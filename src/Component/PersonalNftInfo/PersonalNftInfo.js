...
const PersonalNftInfo = ({
  ...
  const otherCurrencyPricing = async (idx, mult, round) => {
    try {
      ...
      pricesOtherCurr = pricesOtherCurr.map((curr) =>
        Number(ethers.utils.formatUnits(curr, mult[idx + 1]))
      );

      console.log(pricesOtherCurr);

      return pricesOtherCurr;
    } catch (err) {
      console.log(err);
    }
  };
    ...
    try {
      ...
      if (Number(approvedAmounts[whichCurr]) >= Number(targetObj.total)) {
        return (
          <>
            <button
              disabled={loaded[whichCurr] === 0 || disable}
